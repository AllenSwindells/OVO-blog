...
?>
    <style>
        ...
        #edit-in-themler:before {
            content: url('<?php echo esc_url(get_template_directory_uri()); ?>/images/static/billon-themes.png');
...
/*
function theme_process_upage_shortcode($atts, $content) {
    $atts = shortcode_atts(array(
        'id' => 0,
    ), $atts);
    $section_id = (int)$atts['id'];
    $post = get_post($section_id);
    if (!$post || $post->post_type !== 'upage_section') {
        $section_id = 0;
    }
    ...
    return '[upage_section id=' . $new_section_id . ']';
}

function theme_process_shortcodes($content) {
    $content = themler_render_shortcode($content, 'upage_section', 'theme_process_upage_shortcode');
    return $content;
}*/
