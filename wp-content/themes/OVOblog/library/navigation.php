...
function theme_get_list_menu($args = array()) {
    ...
    foreach ((array) $menu_items as $key => $menu_item) {
        ...
        if ($menu_item->object_id == $queried_object_id &&
            ...
            $active_id = $menu_item->ID;
        } else if ('custom' == $menu_item->object && theme_urls_equal(theme_get_current_url(), $menu_item->url)) {
            if (!$active_id || !theme_get_array_value(parse_url($menu_item->url), 'fragment')) {
                $active_id = $menu_item->ID;
            }
        }
    }
    ...
    foreach ($sorted_menu_items as $el) {
        ...
        $items[] = new Theme_MenuItem(array(
            ...
            'submenu_item_start_function' => $args['submenu_item_start_function'],
            'submenu_item_end_function' => $args['submenu_item_end_function'],
            'allow_megamenu' => isset($el->theme_allow_megamenu) ? $el->theme_allow_megamenu : '',
            'megamenu_width' => isset($el->theme_megamenu_width) ? $el->theme_megamenu_width : '',
            'megamenu_custom_width' => isset($el->theme_megamenu_custom_width) ? $el->theme_megamenu_custom_width : '',
            'megamenu_columns' => array(
                'lg' => isset($el->theme_megamenu_columns_lg) ? $el->theme_megamenu_columns_lg : '',
                'md' => isset($el->theme_megamenu_columns_md) ? $el->theme_megamenu_columns_md : '',
                'sm' => isset($el->theme_megamenu_columns_sm) ? $el->theme_megamenu_columns_sm : '',
                'xs' => isset($el->theme_megamenu_columns_xs) ? $el->theme_megamenu_columns_xs : '',
            ),
        ));
    }
...
function theme_get_list_pages($args = array()) {
    ...
    if (function_exists('is_woocommerce') && is_woocommerce() && is_shop()) {
        $current_id = (int)woocommerce_get_page_id('shop');
    }
