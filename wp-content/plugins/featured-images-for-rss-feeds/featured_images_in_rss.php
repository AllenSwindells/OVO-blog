...
function firss_init()
{
    ...
    // Premium.
    if ( fifrf_fs()->is_paying__premium_only() || fifrf_fs()->is_trial() ) {
        require_once dirname( __FILE__ ) . '/includes/premium/premium.php';
    }
}
...
{
    $locale = apply_filters( 'plugin_locale', get_locale(), 'featured_images_in_rss' );
    load_textdomain( 'featured_images_in_rss', WP_LANG_DIR . '/plugins/featured-images-in-rss-' . $locale . '.mo' );
    load_plugin_textdomain( 'featured_images_in_rss', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
}
...
function firss_settings_page()
{
	...
	<div class="wrap">
		<h2></h2>
		<div class="headerDiv" >
			<img class="headerImg" src="https://ps.w.org/featured-images-for-rss-feeds/assets/banner-with-title.jpg" width="772">
		</div>
    ...
    ?>
		        ...
		        <tr valign="top">
		            <th scope="column"><?php 
    echo  __( 'Set the size of feed images', 'featured_images_in_rss' ) ;
    ?>
</th>
		            <td>
			            <?php 
...
>full</option>
    ...
    echo  __( 'Width', 'featured_images_in_rss' ) ;
    ?>
</label>
...
" class="small-text">
    ...
    echo  __( 'Height', 'featured_images_in_rss' ) ;
    ?>
</label>
...
" class="small-text"> px
						...
						<p>
							<small><?php 
    echo  sprintf( __( '(Customize image pixel sizes in <a href="%1$s">Media Options</a>, then you\'ll need to <a href="%2$s" target=_blank>Regenerate Thumbnails</a>.)', 'featured_images_in_rss' ), '/wp-admin/options-media.php', 'http://wordpress.org/plugins/regenerate-thumbnails/' ) ;
    ?>
							</small>
						</p>
		            </td>
		        ...
		        <tr>
		            <th scope="column"><?php 
    echo  __( 'Set alignment of feed images', 'featured_images_in_rss' ) ;
    ?>
</th>
		            <td>
    ...
    ?>
><?php 
    echo  __( 'Image Left Above Text', 'featured_images_in_rss' ) ;
    ?>
</option>
    ...
    ?>
><?php 
    echo  __( 'Image Centered Above Text', 'featured_images_in_rss' ) ;
    ?>
</option>
    ...
    ?>
><?php 
    echo  __( 'Image Left Text Wraps', 'featured_images_in_rss' ) ;
    ?>
</option>
    ...
    ?>
><?php 
    echo  __( 'Image Right Text Wraps', 'featured_images_in_rss' ) ;
    ?>
</option>
		                </select>
		            </td>
		        ...
		        <tr>
		            <th scope="column"><?php 
    echo  __( 'Set the spacing between text and feed images', 'featured_images_in_rss' ) ;
    ?>
</th>
		            <td>
...
" /></p>
    ...
    ?>
		...
		<div class="footer-notes">
			<br/>- <?php 
    echo  sprintf( __( 'If you like this plugin, please <a href="%s" target=_blank>Rate and Review</a> it so others can benefit too.', 'featured_images_in_rss' ), 'http://wordpress.org/support/view/plugin-reviews/featured-images-for-rss-feeds/?rate=5#new-post' ) ;
    ?>
			<br/>- <?php 
    echo  sprintf( __( 'Still not seeing images in your feed? Verify featured images are set. Clear your browser and any server caches. Using Feedburner? Be sure to also <a href="%s" target=_blank>Ping Feedburner</a> so it refreshes your feed.', 'featured_images_in_rss' ), esc_url( 'http://feedburner.google.com/fb/a/pingSubmit?bloglink=' . site_url() ) ) ;
    ?>
			<br/>- <?php 
    echo  'To view your site’s raw RSS feed, click here: <a href="' . site_url() . '/feed/" target=_blank>/feed/</a>' ;
    ?>
			<br/>- <?php 
    echo  sprintf( __( 'Need help? Please <a href="%s" target=_blank>submit a new Support Thread</a>', 'featured_images_in_rss' ), 'http://wordpress.org/support/plugin/featured-images-for-rss-feeds' ) ;
    ?>
			<h4><?php 
    echo  sprintf( __( 'Author: Rob Marlbrough, <a href="%1$s" target=_blank>%2$s</a>', 'featured_images_in_rss' ), 'http://presswizards.com/wordpress/', 'Press Wizards WordPress Development' ) ;
    ?>
</h4>
			<h4><?php 
    echo  sprintf( __( 'Co-author: <a href="%1$s" target=_blank>%2$s</a>', 'featured_images_in_rss' ), 'http://fandommarketing.com/', 'Fandom Marketing Social and Digital Marketing Agency' ) ;
    ?>
</h4>
			<h4><?php 
    echo  sprintf( __( 'We can help with WordPress design, maintenance, and content marketing. <a href="%s" target=_blank>Contact us</a>', 'featured_images_in_rss' ), 'http://presswizards.com/wordpress/' ) ;
    ?>
</h4>
...
function firss_plugin_meta_links( $links, $file )
{
    ...
    // Create link.
    if ( $file == $plugin ) {
        return array_merge( $links, array( '<a href="https://wordpress.org/support/plugin/featured-images-for-rss-feeds/reviews/?rate=5#new-post" target=_blank>' . __( 'Please rate and review', 'featured_images_in_rss' ) . '</a>' ) );
    }
    return $links;
}
...
function firss_inform_premium()
{
	...
	<table class="form-table premium-features">
		<tr class="premiumHead">
			<th class="preBanner" scope="column" colspan=2>
				<h1>
					<?php 
    echo  sprintf( __( 'Upgrade for these Premium Features%s', 'featured_images_in_rss' ), ( fifrf_fs()->is_trial() ? ' ' . __( '(Free Trial)', 'featured_images_in_rss' ) : '' ) ) ;
    ?>
				</h1>
			</th>
		</tr>
    ...
    ?>
		<tr>
			<th style="text-align: center; padding-bottom: 20px;" scope="column" colspan="2"><a class="btnBuy" href="/wp-admin/admin.php?page=featured-images-for-rss-feeds-pricing&checkout=true&plan_id=261&plan_name=premium&billing_cycle=annual&licenses=1">Upgrade Now</a>
			</th>
		</tr>
	</table>
<?php 
...
function firss_call_to_action()
{
	...
	<div class="call-to-action">
		<a class="btnBuy" href="/wp-admin/admin.php?page=featured-images-for-rss-feeds-pricing&checkout=true&plan_id=261&plan_name=premium&billing_cycle=annual&licenses=1">Upgrade Now</a>
		<?php 
...
function firss_eval_css( $featured_images_in_rss_css )
{
    ...
    switch ( $featured_images_in_rss_css ) {
        case 'left-above':
            $featured_images_in_rss_css_code = 'display: block; margin-bottom: %1$spx; clear:both;';
            break;
        case 'centered-above':
            $featured_images_in_rss_css_code = 'display: block; margin: auto; margin-bottom: %1$spx;';
            break;
        case 'left-wrap':
...
{
    $features = array(
        __( 'Media Tag Support', 'featured_images_in_rss' )         => __( 'Support for media and image tags.', 'featured_images_in_rss' ),
        __( 'Use First Body Image', 'featured_images_in_rss' )      => __( 'Use first image in post as images in RSS feed.', 'featured_images_in_rss' ),
        __( 'Disable responsive images', 'featured_images_in_rss' ) => __( 'Disable responsive images (fixes images in certain RSS readers).', 'featured_images_in_rss' ),
        __( 'Premium Support', 'featured_images_in_rss' )           => __( 'Priority support to help you with features, configuration or use.', 'featured_images_in_rss' ),
        __( 'Custom Image Size', 'featured_images_in_rss' )         => __( 'Set custom sizes of images in RSS feed.', 'featured_images_in_rss' ),
        __( 'Exclude Categories', 'featured_images_in_rss' )        => __( 'Exclude certain categories from feed.', 'featured_images_in_rss' ),
        __( 'Pre-Feed Content', 'featured_images_in_rss' )          => __( 'Insert text or HTML messages, ads, links before your feed.', 'featured_images_in_rss' ),
        __( 'Post-Feed Content', 'featured_images_in_rss' )         => __( 'Also insert text or HTML messages, ads, links after your feed.', 'featured_images_in_rss' ),
        __( 'Publish Delay', 'featured_images_in_rss' )             => __( 'Delays the publishing of the feed after a post is published.', 'featured_images_in_rss' ),
    );
    return $features;
}
