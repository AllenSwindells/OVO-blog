...
function theme_get_preview_nonce() {
    return wp_create_nonce('preview-customize_' . get_stylesheet() . '_preview');
}
...
function getThemeTemplates(){
    ...
    if (theme_woocommerce_enabled()){
        $cart_page  = (int) woocommerce_get_page_id('cart');
        $templates['cart_url'] = 'p=' . $cart_page . '&';
        $pages[] = $cart_page;
        $pages[] = (int) woocommerce_get_page_id('shop'); // shop page
    }
...
$theme_editor_messages = array(
    'preview_edit' => '<p><b>Unable to open theme.</b></p>'.
        ...
        '<li>You may need to <a href="http://answers.themler.com/articles/14063/how-to-clear-appcache">clear HTML 5 App Cache</a>.</li>'.
        '</ol>',
